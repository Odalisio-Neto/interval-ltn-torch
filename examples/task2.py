...
def plot_function(name):
    ...
    ax.set_yticks([0.,1.])
    iltn.utils.plot.plot_events(np.arange(0.,trace_length,0.1), [A,B,C], ax, zorders=[1,2,1])
    ax.legend()
...
def constraints(training:bool = True):
    ...
    cstr2 = iltn.utils.ops.smooth_equal(trapz_ops.duration(B),1.5)
    return ltn_ops.Forall(tf.stack([cstr1, cstr2]))
constraints()
