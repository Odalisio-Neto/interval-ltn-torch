...
from iltn.events.trapz import (TrapzEvent, LeftInfiniteTrapzEvent, RightInfiniteTrapzEvent, LeftFiniteTrapezoidalEvent, RightFiniteTrapezoidalEvent)
...
class Contains:
    ...
    def __call__(self, 
        ...
        if isinstance(A, LeftInfiniteTrapzEvent):
            bound = B.a.detach()  # Replacing tf.stop_gradient with PyTorch equivalent
    ...
    def __call__(self, A: TrapzEvent, B: LeftFiniteTrapezoidalEvent, smooth: bool = None
        ...
        if isinstance(B, RightInfiniteTrapzEvent):
            ...
            return self.fuzzy_and(
                self.equals(self.op_start(A), self.op_start(B), smooth=smooth), 
