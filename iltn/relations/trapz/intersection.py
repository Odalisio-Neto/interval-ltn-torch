...
class EdgeLine:
    ...
    inv_slope: float | tf.Tensor
    xintercept: float | tf.Tensor
    ...
    def get_x(self, y: float | tf.Tensor) -> float | tf.Tensor:
        return self.inv_slope*y + self.xintercept

    def get_y(self, x: float | tf.Tensor) -> float | tf.Tensor:
        if self.is_vertical():
...
def shoelace_formula(vertices: list[Point]) -> float | tf.Tensor:
    """
    Args:
        vertices (list[Point]): Must be in counter-clockwise order
    """
    xs = tf.stack([v.x for v in vertices])
    ys = tf.stack([v.y for v in vertices])
    s1 = tf.reduce_sum(xs*tf.roll(ys,-1,axis=0))
    s2 = tf.reduce_sum(ys*tf.roll(xs,-1,axis=0))
    area = 0.5*tf.math.abs(s1-s2)
    return area
...
def find_intersection_vertices(A: TrapzEvent, B: TrapzEvent) -> list[tuple[float, float]]:
    ...
    if A.d <= B.a:
        return []
    # Bottom vertices
    bottom_vertices = [Point(B.a, 0.), Point(tf.minimum(A.d,B.d), 0.)]
    # Top vertices
    if A.c < B.b or A.b > B.c:
        ...
        top_vertices = [Point(tf.minimum(A.c,B.c), 1.), Point(tf.maximum(A.b,B.b), 1.)] # order matters
    # side vertices
